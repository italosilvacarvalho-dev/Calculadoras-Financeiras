...
if(localStorage.getItem(THEME_KEY)==="1"){ document.body.classList.add("light"); }
themeToggle?.addEventListener("click", ()=>{
  document.body.classList.toggle("light");
  localStorage.setItem(THEME_KEY, document.body.classList.contains("light") ? "1":"0");
});
...
registerCalc("juros-compostos", (function(){
  ...
  function exportar(){
    ...
    const head = ["Mês","Aporte","Juros do mês","Saldo ao final"];
    const rows = linhas.map(l => [l.mes, l.aporte, l.juros, l.saldo]);
    rows.unshift(head);
    rows.push([]); rows.push(["Montante","","",montante]);
    rows.push(["Total Aportado","","",totalAportes]);
    rows.push(["Juros Acumulados","","",jurosAcum]);
    const csv = rows.map(r=>r.map(v=>typeof v==="number"?String(v).replace(".",","):String(v)).join(";")).join("\n");
...
registerCalc("poupanca-selic", (function () {
  ...
  function exportCSV(){
    if(!last) return;
    const rows = [["Mês","Poupança","Selic (bruta)","Selic (líquida)"]];
    for(let i=0;i<last.labels.length;i++){
      rows.push([ last.labels[i], last.seriePoup[i], last.serieSelicBruta[i], last.serieSelicLiq[i] ]);
    }
    rows.push([]);
    rows.push(["Montante Poupança","","","", last.finais.poup]);
    rows.push(["Montante Selic (bruta)","","","", last.finais.selicBruta]);
    rows.push(["Montante Selic (líquida)","","","", last.finais.selicLiq]);
    const csv = rows.map(r=>r.map(v=> typeof v==="number" ? String(v).replace(".",",") : String(v)).join(";")).join("\n");
    const url = URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"}));
